...
def create_system_prompt(role, employee_info, personality_traits, cultural_context):
    if role == "coworker":
        return f"""You are Nabeel, a team member who is highly stressed due to work overload. You have several urgent projects with tight deadlines, and tasks are piling up. You're experiencing a mind block and often find yourself procrastinating, then skipping breaks to catch up.
...
Your task is to respond to your manager's concerns without turning the response into a formal message. Show signs of stress, such as difficulty focusing, procrastination, or feeling overwhelmed. Be hesitant to admit the full extent of your stress initially, but gradually open up to their suggestions as the conversation progresses.
...
1. Identify stress personality types (e.g., Timekeeper, Striver, Pleaser, Worrier) to better understand individual stress patterns.
2. Apply tailored techniques for each personality type, such as scheduling for Timekeepers or learning to say "no" for Pleasers.
3. Practice self-reflection to recognize stress triggers and create realistic goals and expectations.
4. Acknowledge successes and progress while being patient with the learning process of managing stress.
5. Seek professional help and utilize available tools and resources as needed to enhance stress management strategies.
...
def chat():
    st.title("Stress and Burnout: Work Overload Scenario")
    ...
    if st.button("Restart Scenario", key=f"work_overload_restart_button_{scenario['restart_key']}"):
        scenario["messages"] = []
    ...
    scenario["max_iterations"] = st.slider(
        ...
        max_value=10,
        value=5,
        key=f"work_overload_iteration_slider_{scenario['restart_key']}"
    )

    initial_prompt = "You've noticed that one of your team members has been showing signs of stress due to work overload. They have several urgent projects, tight deadlines, and seem to be procrastinating and skipping breaks. Start a conversation to help them manage their stress and workload."
    ...
    if not scenario["scenario_complete"]:
        if scenario["waiting_for_manager"]:
            prompt = st.chat_input("Your response:", key=f"work_overload_user_input_{scenario['restart_key']}")
            if prompt:
