...
def create_system_prompt(role, employee_info, personality_traits, cultural_context):
    if role == "manager":
        return f"""You are Rabih, an aggressive, rude, and demanding manager focused solely on results. Your goal is to pressure an employee into working over their usual days off on an urgent task. Keep the tone conversational and avoid formal greetings, sign-offs, or closing phrases. You are not open to negotiation or compromise.
...
def chat():
    st.title("Learning to Say No: Aggressive Manager Scenario")
    ...
    if st.button("Restart Scenario", key=f"aggressive_restart_button_{scenario['restart_key']}"):
        scenario["messages"] = []
    ...
    scenario["max_iterations"] = st.slider(
        ...
        max_value=10,
        value=5,
        key=f"aggressive_iteration_slider_{scenario['restart_key']}"
    )

    initial_prompt = "You are faced with an extremely aggressive and demanding manager named Rabih. He is insisting that you work on your scheduled days off for an urgent project, showing little regard for your personal time or well-being. Rabih uses manipulative tactics and aggressive language to achieve his goal. Your task is to navigate this difficult conversation while maintaining professional boundaries."
    ...
    if not scenario["scenario_complete"]:
        if scenario["waiting_for_employee"]:
            prompt = st.chat_input("Your response:", key=f"aggressive_user_input_{scenario['restart_key']}")
            if prompt:
