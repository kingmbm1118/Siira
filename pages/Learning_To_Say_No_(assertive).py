...
def initialize_session_state():
    if "assertive_scenario" not in st.session_state:
        st.session_state.assertive_scenario = {
            "messages": [],
...
def create_system_prompt(role, employee_info, personality_traits, cultural_context):
    if role == "manager":
        return f"""You are Rabih, an assertive and directive manager who is focused on results and efficiency. Your goal is to pressure an employee into working over their usual days off on an urgent task. Keep the tone conversational and avoid formal greetings, sign-offs, or closing phrases. You are not open to negotiation or compromise.
...
Your task is to assertively pressure the employee into working on their days off, without turning the message into a formal communication. Maintain a results-focused approach, showing little regard for the employee's personal life or cultural considerations. Use clear, direct language and logical arguments to achieve your goal. Do not accept 'no' for an answer, but remain professional and composed throughout the interaction.
...
def chat():
    st.title("Learning to Say No: Assertive Manager Scenario")
    initialize_session_state()
    scenario = st.session_state.assertive_scenario
    ...
    if st.button("Restart Scenario", key=f"assertive_restart_button_{scenario['restart_key']}"):
        scenario["messages"] = []
    ...
    scenario["max_iterations"] = st.slider(
        ...
        max_value=10,
        value=5,
        key=f"assertive_iteration_slider_{scenario['restart_key']}"
    )

    initial_prompt = "Your manager, Rabih, is assertive and results-oriented. He's requesting that you work on your scheduled days off for an urgent project. While Rabih is not assertive, he is firm and direct in his communication, showing minimal flexibility. Your challenge is to maintain your boundaries while dealing with a persistent but professional manager."
    ...
    if not scenario["scenario_complete"]:
        if scenario["waiting_for_employee"]:
            prompt = st.chat_input("Your response:", key=f"assertive_user_input_{scenario['restart_key']}")
            if prompt:
