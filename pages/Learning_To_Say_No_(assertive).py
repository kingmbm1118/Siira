...
def create_system_prompt(role, employee_info, personality_profile, cultural_context):
    ...
    if role == "manager":
        return f"""You are Rabih, an assertive but professional manager requesting an employee to work during their time off.
...
1. Present clear business needs
2. Maintain professional tone
3. Be persistent but respectful
4. Focus on project importance
5. Acknowledge but minimize personal impact
6. Emphasize team responsibility
...
- Express urgency professionally"""
    elif role == "therapist":
        return f"""As a therapist specializing in workplace boundaries, provide feedback to {employee_info['name']} on their response to assertive requests.
...
Focus feedback on:
1. Professional boundary setting
...
ACCEPT: Agreeing without establishing boundaries
...
ACCEPT: 2-3 stars (missed boundary opportunity)
...
def analyze_employee_response(response):
    prompt = f"""Analyze this response to an assertive work request:

Response to analyze: "{response}"

Evaluate against:
1. ACCEPT: Agrees without setting boundaries
...
def display_scenario_header():
    ...
    <div style="background-color: var(--card-background, #f8f9fa); 
        ...
        <h3 style="color: var(--text-color, #495057); 
                   margin-bottom: 15px;">
            Scenario: Handling an Assertive Manager
        </h3>
        <p style="color: var(--text-color, #6c757d); 
                  font-size: 16px; 
                  line-height: 1.6;">
            Your manager, Rabih, is firmly requesting you to work during your scheduled time off. 
            While professional, they are persistent and focused on business needs. Your challenge is to:
...
def display_completion_message(scenario):
    ...
    messages = {
        ...
        "decline": "You've professionally maintained your boundaries.",
...
def chat():
    ...
    with col1:
        if st.button("🔄 Restart Scenario", key=f"assertive_restart_{scenario['restart_key']}"):
            scenario["messages"] = []
    ...
    with col2:
        scenario["max_iterations"] = st.select_slider(
            ...
            value=5,
            key=f"assertive_rounds_{scenario['restart_key']}"
        )
    ...
    if not scenario["scenario_complete"]:
        if scenario["waiting_for_employee"]:
            user_input = st.chat_input("Your response:", 
                                     key=f"assertive_input_{scenario['restart_key']}")
            if user_input:
