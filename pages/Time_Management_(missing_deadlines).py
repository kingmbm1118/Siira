...
Your task is to respond to your manager's concerns about your missed deadlines. Show some understanding of the issue, but also provide reasons or excuses for your behavior. Gradually become more open to suggestions and solutions as the conversation progresses.
...
1. Adapt to different time management styles by identifying the most effective approach for your work habits.
2. Use tools like schedules, calendars, and to-do lists to organize and prioritize tasks efficiently.
3. Break large projects into smaller tasks and prioritize them based on urgency and importance.
4. Implement time management techniques such as time-blocking, the two-minute rule, and the Pomodoro Technique to stay focused.
5. Minimize distractions and delegate tasks where possible to maintain productivity and focus.
6. Set realistic deadlines with buffer time, and regularly review your strategies for improvement.
7. Take regular breaks to maintain energy, and conduct time audits to identify inefficiencies.
...
Conclude the assessment with a rating out of 5 stars, formatted as [RATING: X], based on their Time and deadlines managment skills, according to the following:
...
def chat():
    st.title("Time Management: Missing deadlines")
    ...
    if st.button("Restart Scenario", key=f"missing_deadlines_restart_button_{scenario['restart_key']}"):
        scenario["messages"] = []
    ...
    scenario["max_iterations"] = st.slider(
        ...
        max_value=10,
        value=5,
        key=f"missing_deadlines_iteration_slider_{scenario['restart_key']}"
    )

    initial_prompt = "You are the project manager of a small team. You've noticed that Ahmad, one of your team members, is consistently missing deadlines and asking for extensions. This is causing problems for other team members who depend on Ahmad's work. Start a conversation with Ahmad to address this issue and find a solution."
    ...
    if not scenario["scenario_complete"]:
        if scenario["waiting_for_manager"]:
            prompt = st.chat_input("Your response:", key=f"missing_deadlines_user_input_{scenario['restart_key']}")
            if prompt:
